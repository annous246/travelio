...
export default function SeasonInput({index,accomodations,setAccomodations,setFromdate,setTodate}){
    ...
    function checkInOverlap(from,to){
      let ok=true;
      if(!date.length)return true
      accomodations.forEach((acc,ind)=>{
        console.log(acc)
        if(ind!=index){
          if(acc.toDate.length&&acc.fromDate.length){
           ...
           let ourFromDate=new Date(from).getTime()
           let ourToDate=new Date(to).getTime()
           let from=new Date(acc.fromDate).getTime()
           let to=new Date(acc.toDate).getTime()
           if(ourFromDate<=from&&ourToDate>=to)ok=false;
          }}
      })
      return ok;
    }
