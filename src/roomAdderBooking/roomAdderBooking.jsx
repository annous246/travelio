...
export default function RoomAdderBooking({supplements, setSupplements, setMembers,setThestate,roomAdderState,setRoomAdderState,setLoading, setAvailableRooms,hotel,availableRooms}){
  console.log("here")
...
console.log(hotel)
    return(
      <div id="main-roomAdder-containere" ref={mainContainer}>
        <div id="roomAdder-containere" ref={container}>
        <roomsContext.Provider value={[allRooms,setAllRooms]}>
        {allRooms.map((room,index)=>{
        return <Room supplements={supplements} setSupplements={setSupplements} SS={hotel.SS} SSStatus={hotel.SSStatus} SVP={hotel.SVP} SVPStatus={hotel.SVPStatus} SVM={hotel.SVM} SVMStatus={hotel.SVMStatus} roomAdderState={roomAdderState} setMembers={setMembers} setAvailableRooms={setAvailableRooms} availableRooms={availableRooms} index={index}  key={room.id}  currentRoom={room}  container={container}  />
        ...
        <button type="button" onClick={it} id="roomAdder-closee">X</button>
        <button type="button" onClick={addRoom} id="roomAdder-adde"></button>
</div>
    )
}